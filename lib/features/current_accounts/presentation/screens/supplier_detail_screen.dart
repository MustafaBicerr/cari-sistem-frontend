...
class _SupplierDetailScreenState extends ConsumerState<SupplierDetailScreen> {
    ...
    return Scaffold(
      ...
      body: detailAsync.when(
        ...
        data: (response) {
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      onPressed:
                          financeState.isLoading || _selectedInvoiceIds.isEmpty
                              ? null
                              : () async {
                                final selectedInvoices =
                                    history
                                        .where(
                                          (h) => _selectedInvoiceIds.contains(
                                            h.id,
                                          ),
                                        )
                                        .toList();
                                final totalRemaining = selectedInvoices
                                    .fold<double>(
                                      0,
          ...
          if (isMobile) {
            // Mobil: 1) Genel Bilgi 2) İletişim 3) Faturalar, alt alta ve dikey scroll
            return Column(
              children: [
                Expanded(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.stretch,
                            children: [
                              _buildSummaryCard(profile, stats),
                        ...
                        _buildInvoiceList(history, embedded: true),
                        const SizedBox(height: 8),
                        // Alt navigasyon barı için ekstra boşluk
                        const SizedBox(height: 16),
                      ],
                    ),
                  ),
          ...
          // Desktop / Web: mevcut yatay yerleşim
          return Column(
            children: [
              Padding(
                padding: const EdgeInsets.all(16),
                child: Row(
                  children: [
                    Expanded(flex: 2, child: _buildSummaryCard(profile, stats)),
                    const SizedBox(width: 16),
                    Expanded(flex: 2, child: _buildContactCard(profile)),
                  ],
                ),
              ),
              const Padding(
              ...
              const SizedBox(height: 8),
              Expanded(child: _buildInvoiceList(history)),
              buildBottomBar(),
            ],
          );
        },
  ...
  Widget _buildInvoiceList(
    List<SupplierHistoryItem> history, {
    bool embedded = false,
  }) {
    if (history.isEmpty) {
      return const Center(child: Text("Henüz fatura kaydı yok."));
    }
        ...
        onTap: () {
          ...
          if (isMobile) {
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => PurchaseInvoiceDetailScreen(invoiceId: item.id),
              ),
            );
          } else {
            showDialog(
              context: context,
              builder: (ctx) => PurchaseInvoiceDetailDialog(invoiceId: item.id),
            );
          }
        },
        child: Container(
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      item.description.isNotEmpty ? item.description : "Fatura",
                      style: const TextStyle(
    ...
    if (embedded) {
      // Mobil dikey scroll içinde kullanırken: kendi scroll'unu kapat, yüksekliği sar.
    ...
    // Desktop / web: bağımsız scroll eden liste
    return ListView.separated(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      itemCount: history.length,
      separatorBuilder: (_, __) => const SizedBox(height: 8),
